...
def get_training_data_sequence(dir_file, filename_no_ext):
    ...
    for i in range(len(tier.annotations)):
        ...
        for j in range(current_ult_data.shape[0]):
            if text != 'start_sil' and text != 'end_sil':
        ...
        for file in os.listdir(dir_file):
            if ".ult" in file:
                try:
                    ...
                    text_list = []
                    ult_data_all = []
                    base_dir = 'E:\\spkr102\\ult_data'
                    for i in range(len(data_sequence)):
                        text_list.append(data_sequence[i]['phone_text'])
                        ult_data_all.append(data_sequence[i]['ult_data'])
                    count = Counter(text_list)
                    for key in count.keys():
                        save_path_temp = base_dir + '\\' +key
                        if os.path.exists(save_path_temp):
                            save_path = save_path_temp
                        else:
                            os.makedirs(save_path_temp)
                            save_path = save_path_temp
                        index_ = np.where(np.array(text_list) == key)[0]
                        print('*************************%s************************'%key)
                        for j in range(len(index_)-2):
                            data1 = index_[j]
                            data2 = index_[j+1]
                            data3 = index_[j+2]
                            temp_data = np.dstack((ult_data_all[data1], ult_data_all[data2], ult_data_all[data3]))
                            id_ = len(os.listdir(save_path))
                            misc.imsave(save_path +'\\'+ str(id_)+'.png', temp_data)
                except ValueError as e:
